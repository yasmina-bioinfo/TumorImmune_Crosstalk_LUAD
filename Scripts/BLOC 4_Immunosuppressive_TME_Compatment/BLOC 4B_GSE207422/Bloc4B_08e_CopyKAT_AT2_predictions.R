#!/usr/bin/env Rscript
# ============================================================
# GSE207422 : Bloc4B Script 08e: CopyKAT on Tumor_epithelial_AT2
# Both MPR and NMPR analyzed together
# Normal reference: Ciliated_epithelial
# Input:  Objects/Bloc4B_07_seu_Epithelial.rds
# Output: Results/Tables/Bloc4B_08e_CopyKAT_AT2_predictions.csv
...
# 1) Load and subset AT2 + Ciliated
...
cells_keep <- colnames(seu_Epi)[seu_Epi$TME_cell_type %in% 
                                  c("Tumor_epithelial_AT2", "Ciliated_epithelial")]
seu_sub <- subset(seu_Epi, cells = cells_keep)
message("AT2 MPR: ", sum(seu_sub$TME_cell_type == "Tumor_epithelial_AT2" & seu_sub$PathResponse == "MPR"))
message("AT2 NMPR: ", sum(seu_sub$TME_cell_type == "Tumor_epithelial_AT2" & seu_sub$PathResponse == "NMPR"))
...
# 4) Run CopyKAT
message("Running CopyKAT on AT2 cells...")
setwd(OUT_FIG)
...
fwrite(predictions, file.path(OUT_TAB, "Bloc4B_08e_CopyKAT_AT2_predictions.csv"))
message("Saved: Bloc4B_08e_CopyKAT_AT2_predictions.csv")
message("DONE CopyKAT AT2")