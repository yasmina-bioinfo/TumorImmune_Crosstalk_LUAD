#!/usr/bin/env Rscript
# ============================================================
# GSE207422 : Bloc4B Script 08d: CopyKAT on Tumor_epithelial_EMT — NMPR only
# EMT cells are exclusive to NMPR condition
# Normal reference: Ciliated_epithelial
# Input:  Objects/Bloc4B_07_seu_Epithelial.rds
# Output: Results/Tables/Bloc4B_08d_CopyKAT_EMT_NMPR_predictions.csv
...
cells_keep <- colnames(seu_Epi)[
  seu_Epi$TME_cell_type %in% c("Tumor_epithelial_EMT", "Ciliated_epithelial")]
seu_sub <- subset(seu_Epi, cells = cells_keep)
message("EMT NMPR: ", sum(seu_sub$TME_cell_type == "Tumor_epithelial_EMT"))
...
fwrite(predictions, file.path(OUT_TAB, "Bloc4B_08d_CopyKAT_EMT_NMPR_predictions.csv"))
message("DONE CopyKAT EMT NMPR")